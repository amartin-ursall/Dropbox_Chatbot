...
Write-Host "  Instalación de Certificado SSL" -ForegroundColor Cyan
...
# ============================================================================
# OPCIÓN 1: Certificado Let's Encrypt con win-acme (PRODUCCIÓN)
# ============================================================================
...
if (-not $SelfSigned) {
    ...
    # Verificar dominio
    if (-not $Domain) {
        Write-Host "ADVERTENCIA: No se especificó dominio" -ForegroundColor Yellow
    ...
    Write-Host "[1/5] Verificando resolución DNS..." -ForegroundColor Green
    try {
        ...
        if ($resolvedIP -eq $localIP) {
            ...
            Write-Host "  Asegúrate de que el DNS esté correctamente configurado" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "  ⚠ No se pudo verificar DNS: $($_.Exception.Message)" -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "  ✓ win-acme instalado en $winAcmePath" -ForegroundColor Green
    } catch {
    ...
    Write-Host ""
    Write-Host "[3/5] Verificando configuración de IIS..." -ForegroundColor Green
    ...
    try {
        ...
        if (-not $site) {
            ...
            Write-Host "  win-acme necesita que IIS responda en puerto 80 para la validación" -ForegroundColor Yellow
        } else {
            Write-Host "  ✓ Sitio IIS encontrado: $($site.Name)" -ForegroundColor Green
        }
    } catch {
        Write-Host "  ⚠ No se pudo verificar IIS" -ForegroundColor Yellow
    }
    ...
    Write-Host "  1. Selecciona opción 'N' (Create certificate - simple mode)" -ForegroundColor Gray
    Write-Host "  2. Ingresa tu dominio: $Domain" -ForegroundColor Gray
    Write-Host "  3. Selecciona IIS como destino de instalación" -ForegroundColor Gray
    Write-Host "  4. Acepta crear tarea programada para renovación automática" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "[5/5] Verificando instalación..." -ForegroundColor Green
    ...
    if ($cert) {
        ...
        Write-Host "  Válido hasta: $($cert.NotAfter)" -ForegroundColor Gray
        Write-Host "  Thumbprint: $($cert.Thumbprint)" -ForegroundColor Gray
    } else {
        Write-Host "  ⚠ No se encontró certificado para $Domain" -ForegroundColor Yellow
    }
    ...
    if ($task) {
        Write-Host ""
        Write-Host "  ✓ Tarea programada de renovación creada" -ForegroundColor Green
        Write-Host "  El certificado se renovará automáticamente cada 60 días" -ForegroundColor Gray
    }

} else {
    # ============================================================================
    # OPCIÓN 2: Certificado Autofirmado (SOLO DESARROLLO/TESTING)
    # ============================================================================

    Write-Host "⚠ ADVERTENCIA: Creando certificado autofirmado" -ForegroundColor Yellow
    Write-Host "Los certificados autofirmados NO son válidos para OAuth de Dropbox" -ForegroundColor Yellow
    Write-Host "Solo úsalos para pruebas internas" -ForegroundColor Yellow
    Write-Host ""
    ...
    try {
        ...
        if ($site) {
            ...
            Write-Host "  ✓ Binding HTTPS configurado en IIS" -ForegroundColor Green
            Write-Host "  Sitio: $($site.Name)" -ForegroundColor Gray
        } else {
            Write-Host "  ⚠ No se encontró sitio en IIS" -ForegroundColor Yellow
            Write-Host "  Deberás configurar el binding HTTPS manualmente" -ForegroundColor Yellow
        }

    } catch {
        Write-Host "  ⚠ No se pudo configurar IIS automáticamente" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  ✓ Instalación SSL Completada" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $SelfSigned) {
    ...
    Write-Host ""
    Write-Host "Este certificado NO funcionará para:" -ForegroundColor Yellow
    Write-Host "  - OAuth de Dropbox desde navegadores externos" -ForegroundColor Gray
    Write-Host "  - Navegadores sin agregar excepción de seguridad" -ForegroundColor Gray
    Write-Host ""
    Write-Host "Para producción, usa Let's Encrypt:" -ForegroundColor Yellow
